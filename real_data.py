...
m_user=np.load(input_path+'user_f_m.npy')
m_item=np.load(input_path+'item_f_m.npy')

# jester=np.load('jester.npy')
...
item_features=m_item[random_item]
...
np.save(path+'movielens_offline_prob_mean_d_%s'%(dimension), offline_prob_mean)
np.save(path+'movielens_offline_prob_std_d_%s'%(dimension), offline_prob_std)
np.save(path+'movielens_online_prob_mean_d_%s'%(dimension), online_prob_mean)
np.save(path+'movielens_online_prob_std_d_%s'%(dimension), online_prob_std)

np.save(path+'movielens_online_beta_mean_d_%s'%(dimension), online_beta_mean)
np.save(path+'movielens_online_beta_std_d_%s'%(dimension), online_beta_std)
...
plt.tight_layout()
plt.savefig(path+'movielens_regret_shadow_soft_d_%s'%(dimension)+'.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig(path+'movielens_error_shadow_soft_d_%s'%(dimension)+'.png', dpi=300)
plt.show()


def find_beta(alpha, sigma, delta, dimension, iteration):
	beta=np.sqrt(alpha)+np.sqrt(sigma)*np.sqrt(dimension*np.log(1+iteration/dimension)+2*np.log(1/delta))
	return beta
...
plt.tight_layout()
plt.savefig(path+'movielens_online_offline_prob'+'.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig(path+'movielens_online_beta'+'.png', dpi=300)
plt.show()
