...
class LINTS():
	def __init__(self, dimension, iteration, item_num, user_feature, item_feature, true_payoffs, alpha, delta, sigma, epsilon):
		...
		self.v=np.sqrt(self.sigma)*np.sqrt(24/self.epsilon*self.dimension*np.log(1/self.delta))
		self.v=1
	...
	def select_arm(self, time):
		...
		sample_theta=np.random.multivariate_normal(mean=self.user_f, cov=(self.v)**2*cov_inv)
