...
def main():
    ...
    movie = pd.read_csv(path+'movies.dat', sep='::', names=['movie_id', 'movie_name', 'tag'], engine='python')
    movie = movie_preprocessing(movie)
    # read the ratings data and merge it with movie data
    rating = pd.read_csv(path+"ratings.dat", sep="::",
                           names=["user_id", "movie_id", "rating", "timestamp"], engine='python')
    data = pd.merge(rating, movie, on="movie_id")
    print('Loading data')

    print('extract feature from our data set')
    streaming_batch, user_feature, actions, reward_list = feature_extraction(data)
    streaming_batch.to_csv(path+"streaming_batch.csv", sep='\t', index=False, header=True)
    user_feature.to_csv(path+"user_feature.csv", sep='\t')
    pd.DataFrame(actions, columns=['movie_id']).to_csv("actions.csv", sep='\t', index=False)
    print('save processed data')
    reward_list.to_csv(path+"reward_list.csv", sep='\t', index=False)

    action_context = movie[movie['movie_id'].isin(actions)]
    action_context.to_csv(path+"action_context.csv", sep='\t', index = False)
