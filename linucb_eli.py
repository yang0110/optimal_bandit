...
class LINUCB_ELI():
	...
	def select_arm(self, time):
		...
		for i in self.item_set:
			...
			self.low_ucb_list[i]=est_y-(self.beta/self.state)*x_norm 
			self.upper_ucb_list[i]=est_y+(self.beta/self.state)*x_norm
	...
	def run(self, iteration):
		...
		for time in range(iteration):
			print('time=%s/%s, item_num=%s ~~~~~~LinUCB ELI'%(time, iteration, len(self.item_set)))
			self.find_beta(time)
