...
class LSE_soft_v():
	def __init__(self, dimension, iteration, item_num, alpha, sigma, step_size_beta, step_size_gamma, weight1):
	...
	def find_log_grad_gamma(self, time):
		...
		for i in range(self.item_num):
			for j in range(self.item_num):	
				if i==j:
					self.gamma_log_grad_matrix[i,j]=self.s_list[j]-self.s_list[j]*np.exp(self.gamma_vector[j]*self.s_list[j])/np.sum(np.exp(self.gamma_vector*self.s_list))
				else:
					self.gamma_log_grad_matrix[i,j]=-self.s_list[j]*np.exp(self.gamma_vector[j]*self.s_list[j])/np.sum(np.exp(self.gamma_vector*self.s_list))
	...
	def train(self, train_loops):
		...
		for l in range(train_loops):
			...
			old_payoff=0
			for time in range(self.iteration):
				# print('Train-loop=%s, time/iteration, %s/%s, beta=%s gamma_vector=%s ~~~~~ LSE-Soft-V'%(l, time, self.iteration, np.round(self.beta, decimals=2), self.gamma_vector))
	...
	def run(self, user_feature, item_features, true_payoffs):
		...
		old_payoff=0
		for time in range(self.iteration):
			# print('Test, time/iteration, %s/%s ~~~~~ LSE-Soft-V'%(time, self.iteration))
			ind, x, y, regret=self.select_arm(time, old_payoff)
			old_payoff=y
			self.update_feature(x, y)
			self.find_log_grad_gamma(time)
		...
		return cum_regret[1:], error_list, self.beta_loop, self.gamma_loop, self.cum_regret_loop, self.soft_max_matrix.T
