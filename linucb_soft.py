...
class LinUCB_soft():
	def __init__(self, dimension, iteration, item_num, user_feature, item_feature, true_payoffs, alpha, sigma, step_size, weight1, loops):
		...
		self.beta_grad=0
		self.gamma_grad=0
		self.new_s_list=np.zeros(self.item_num)
		self.new_s_list_grad=np.zeros(self.item_num)
	...
	def select_arm(self, time, old_payoff):
		...
		for i in range(self.item_num):
			...
			self.s_list[i]=np.dot(self.user_f, x)+self.beta*np.sqrt(np.dot(np.dot(x, cov_inv),x))
	...
	def find_log_grad(self, time):
		...
		for i in range(self.item_num):
			self.beta_log_grad_matrix[time, i]=self.gamma*self.x_norm_list2[i]-a/b-self.weight1*self.x_norm_list[i]
	...
	def update_gamma(self, time):
		...
		# 	gamma_grad+=temp_1
		self.gamma+=self.step_size*self.gamma_grad
	...
	def update_beta(self, time):
		...
		# 		pass 
		# 	else:
		temp_2=np.sum([a*b*c for a,b,c in zip(self.est_y_matrix[time], self.soft_max_matrix[time], self.beta_log_grad_matrix[time])])
		...
		# 	beta_grad+=temp_2
		self.beta+=self.step_size*self.beta_grad
		# if self.beta<0:
		# 	self.beta=0
