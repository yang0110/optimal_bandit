...
# np.random.seed(2018)
mush=pd.read_csv(input_path+'mushroom.data')
...
for l in range(loop):
	...
	linucb_model=LINUCB(dimension, iteration, item_num, user_feature,item_features, true_payoffs, alpha, delta, sigma*25, beta)
...
np.save(path+'real_offline_prob_mean_d_%s'%(dimension), offline_prob_mean)
np.save(path+'real_offline_prob_std_d_%s'%(dimension), offline_prob_std)
np.save(path+'real_online_prob_mean_d_%s'%(dimension), online_prob_mean)
np.save(path+'real_online_prob_std_d_%s'%(dimension), online_prob_std)

np.save(path+'real_online_beta_mean_d_%s'%(dimension), online_beta_mean)
np.save(path+'real_online_beta_std_d_%s'%(dimension), online_beta_std)
